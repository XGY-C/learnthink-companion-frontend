...
import { Timer, Document, Medal, WarningFilled, Right } from '@element-plus/icons-vue'
...
const metrics = computed(() => {
  ...
  return [
    ...
    { label: '薄弱项', value: `${stats.value.prevWeakCount}→${stats.value.weakCount}`, unit: '改善中', icon: WarningFilled, color: stats.value.weakCount < stats.value.prevWeakCount ? 'var(--lt-success)' : 'var(--lt-danger)' },
  ]
})
...
<template>
  <div class="max-w-3xl">
    ...
    <template v-else-if="stats">
      ...
      <!-- 雷达图 -->
      <div v-if="stats.radarData.length > 0" class="rounded-xl border p-4 mb-4" style="border-color: var(--lt-border); background-color: var(--lt-bg-card);">
      ...
      <!-- 学习轨迹柱状图 -->
      <div v-if="stats.weeklyActivity.length > 0" class="rounded-xl border p-4 mb-4" style="border-color: var(--lt-border); background-color: var(--lt-bg-card);">
